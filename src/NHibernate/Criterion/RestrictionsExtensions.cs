...
namespace NHibernate.Criterion
{
	public static class RestrictionExtensions
	{
		...
		{
			ExpressionProcessor.ProjectionInfo projection = ExpressionProcessor.FindMemberProjection(methodCallExpression.Arguments[0]);
			string value = ExpressionProcessor.FindValue<string>(methodCallExpression.Arguments[1]);
			MatchMode matchMode = ExpressionProcessor.FindValue<MatchMode>(methodCallExpression.Arguments[2]);
		...
		public static ICriterion ProcessIsLikeMatchModeEscapeChar(MethodCallExpression methodCallExpression)
		{
			...
			string value = ExpressionProcessor.FindValue<string>(methodCallExpression.Arguments[1]);
			MatchMode matchMode = ExpressionProcessor.FindValue<MatchMode>(methodCallExpression.Arguments[2]);
			char? escapeChar = ExpressionProcessor.FindValue<char?>(methodCallExpression.Arguments[3]);
		...
		{
			ExpressionProcessor.ProjectionInfo projection = ExpressionProcessor.FindMemberProjection(methodCallExpression.Arguments[0]);
			object[] values = ExpressionProcessor.FindValue<object[]>(methodCallExpression.Arguments[1]);
		...
		{
			ExpressionProcessor.ProjectionInfo projection = ExpressionProcessor.FindMemberProjection(methodCallExpression.Arguments[0]);
			var values = ExpressionProcessor.FindValue<ICollection>(methodCallExpression.Arguments[1]);
			return projection.Create<ICriterion>(s => Restrictions.In(s, values), p => Restrictions.In(p, values));
		}
