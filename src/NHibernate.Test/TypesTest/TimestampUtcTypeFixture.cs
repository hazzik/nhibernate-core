...
namespace NHibernate.Test.TypesTest
{
	...
	{
		readonly TimestampUtcType Type = (TimestampUtcType)NHibernateUtil.TimestampUtc;
		readonly DateTime Utc = new DateTime(1976, 11, 30, 10, 0, 0, 300, DateTimeKind.Utc);
		readonly DateTime Local = new DateTime(1976, 11, 30, 10, 0, 0, 300, DateTimeKind.Local);
		readonly DateTime Unspecified = new DateTime(1976, 11, 30, 10, 0, 0, 300, DateTimeKind.Unspecified);
		...
		public void Next()
		{
			...
			var next = (DateTime)Type.Next(current, null);
		...
		public void Seed()
		{
			var type = (TimestampUtcType) NHibernateUtil.TimestampUtc;
		...
		[TestCase(623537928003000000, DateTimeKind.Utc, Result="1976-11-30T10:00:00.3000000Z")]
		[TestCase(623537928003000000, DateTimeKind.Unspecified,  Result="1976-11-30T10:00:00.3000000")]
		[TestCase(623537928003000000, DateTimeKind.Local, Result="1976-11-30T10:00:00.3000000+01:00",
			//Ignore=true, IgnoreReason="Offset depends on which system this test is run and can currently now be influenced via the .net framework",
		...
		public void ExpectedIsEqualDotnetFrameworkBehavior()
		{
			...
			Assert.AreEqual(Utc, Local, AssertMessage);
			Assert.AreEqual(Utc, Unspecified, AssertMessage);
			Assert.AreEqual(Unspecified, Local, AssertMessage);
		}
	}
}